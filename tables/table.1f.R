...
m <- read.table("flat.genes.no_peak.variable.stable.tsv", h=F, sep="\t")
...
m$no_peak_frac <- round((m$no_peak / 2666)*100, 2)
m$variable_frac <- round((m$variable / 2666)*100, 2)
m$stable_frac <- round((m$stable / 2666)*100, 2)
...
# 7. save table
pdf("~/public_html/Borsari_et_al_transdifferentiation_chromatin/single_figures/table.1f.pdf", 
    width = 10)
...
print(xtable(m[, 1:4], caption = "stably expressed genes - 2666"), include.rownames=FALSE)
