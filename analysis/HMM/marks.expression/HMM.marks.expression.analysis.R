...
# 6. change to HMM wd
setwd("/no_backup/rg/bborsari/projects/ERC/human/2018-01-19.chip-nf/Borsari_et_al/analysis/HMM/marks.expression/")


# 7. analysis of HMM results for the different states
pdf("~/public_html/Borsari_et_al_transdifferentiation_chromatin/HMM/HMM.marks.expression.pdf",
    width = 15, height = 15)
for (i in 2:20) {
  ...
  m1 <- m1[, paste0("Re", 1:10, "..Intercept.")]
  
  # 7.3. update colnames
  colnames(m1) <- c("expression", "H3K4me1", "H3K4me2", "H3K27ac", "H3K9ac", "H3K4me3",
                    "H3K36me3", "H4K20me1", "H3K9me3", "H3K27me3")
  
  # 7.4. sort states according to expression value
  m1 <- m1[order(m1$expression), ]
  z <- paste0(letters[1:i], ":", gsub("St", "", rownames(m1)))
  names(z) <- rownames(m1)
  
  # 7.5. rescale expression and marks values
  # to range 0-1
  m1[, 2:10] <- as.data.frame(apply(m1[, 2:10], 2, rescale))
  m1$states <- z
  ...
  # 7.6. make plot
  p1 <- as.grob(pheatmap(m1[, 2:10], cluster_cols = F, cluster_rows = F,
                         ...
                         labels_row = z,
                         annotation_row = m1[, c("expression", "states"), drop=F],
