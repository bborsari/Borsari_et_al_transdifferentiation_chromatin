...
all.marks.groups <- all.marks.groups[rownames(all.marks.groups) %in% cluster1$V1, ]
...
# 7. barplot of groups
df.barplot <- data.frame(stringsAsFactors = F)
for ( i in marks) {
  tmp.df <- as.data.frame(t(table(all.marks.groups[all.marks.groups[, i] != "positively_correlated", i])/
                              sum(table(all.marks.groups[all.marks.groups[, i] != "positively_correlated", i]))))
  
  tmp.df$mark <- i
  df.barplot <- rbind(df.barplot, tmp.df)
}
df.barplot$Var1 <- "cluster1"
df.barplot$mark <- factor(df.barplot$mark, levels = c("H3K4me1",
                                                      "H3K4me2",
                                                      "H3K27ac",
                                                      "H3K9ac",
                                                      "H3K4me3",
                                                      "H3K36me3",
                                                      "H4K20me1"))
df.barplot$Var2 <- factor(df.barplot$Var2, levels = c("negatively_correlated",
                                                      "not_correlated",
                                                      "stable",
                                                      "mark_ant",
                                                      "no_diff",
...
pdf("~/public_html/Borsari_et_al_transdifferentiation_chromatin/single_figures/fig.8c.pdf", 
    height = 6, width = 5)
ggplot(df.barplot, 
