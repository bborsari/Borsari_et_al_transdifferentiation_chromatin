...
m <- read.table("H3K4me3/QN.merged/ant.del.analysis/increases/expression.25.50.75.100.tsv",
                h=T, sep="\t")
colnames(m) <- c("25%", "50%", "75%", "100%")
...
clusters <- clusters[clusters$gene_id %in% 
                       rownames(metadata[metadata$final_class == "upregulation", ]), ]
