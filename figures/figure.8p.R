...
# 5. retrieve up-regulated genes
x <- rownames(metadata[metadata$final_class == "upregulation", ])


# 6. read expression matrix & keep only up-regulated genes
expression.m <- read.table("H3K4me3/QN.merged/expression.matrix.tsv", h=T, sep="\t", 
                           stringsAsFactors = F)

# 7. subset expression matrix for up-regulated genes 
...
for (i in 1:7) {
  ...
  # 9.2. retrieve genes that are up-regulated and positively correlated 
  # with expression for this mark
  genes <- read.table(paste0(marks[i], "/QN.merged/", marks[i], ".6.groups.tsv"),
                      h=T, sep="\t", stringsAsFactors = F)
  genes <- rownames(genes[genes$final_class == "upregulation" &
...
# 10.3. save plot
pdf("~/public_html/Borsari_et_al_transdifferentiation_chromatin/single_figures/fig.8p.pdf",
    width=16, height=8)
