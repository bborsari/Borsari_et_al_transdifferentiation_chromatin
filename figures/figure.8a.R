...
my.function2 <- function(mark, cluster) {
  ...
  upreg.genes <- upreg.genes[upreg.genes %in% cluster.genes$V1]
  
  expression.mark.m <- expression.mark.m[expression.mark.m$gene_id %in% upreg.genes, ]
...
# plot 100%
df.100 <- my.plot.df[my.plot.df$degree == "perc_100" & my.plot.df$variable == "expression", c("gene_id", "value", "cluster")]
df.100 <- unique(df.100)
df.100$degree <- "100%"


df <- rbind(df.25, df.50, df.75, df.100)
df$degree <- factor(df$degree, levels = c("25%",
                                          "50%",
                                          "75%",
                                          "100%"))
...
pdf("~/public_html/Borsari_et_al_transdifferentiation_chromatin/single_figures/fig.8a.pdf",
    width = 8, height = 4)
