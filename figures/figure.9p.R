...
expression.m$FC <- expression.m$H000 - expression.m$H168
expression.m$gene_id <- rownames(expression.m)


# 5. retrieve sets of down-regulated genes
...
# 6. subset expression matrix for down-regulated genes
...
for (i in 1:3) {
  ...
  if ( i == 1 | i == 2) { lop[[i]] <- lop[[i]] + ylim(0, 20) } 
  else { lop[[i]] <- lop[[i]] + ylim(0, 11) } 
...
# 12. save plots
pdf("~/public_html/Borsari_et_al_transdifferentiation_chromatin/single_figures/fig.9p.pdf",
    height = 4, width = 8)
