...
all.marks.groups <- all.marks.groups[rownames(all.marks.groups) %in% cluster3$V1, ]
...
for ( i in marks) {
  tmp.df <- as.data.frame(t(table(all.marks.groups[, i])/
                              sum(table(all.marks.groups[, i]))))
  tmp.df$mark <- i
  df.barplot <- rbind(df.barplot,
                      tmp.df)
}
df.barplot$Var1 <- "cluster3"
...
pdf("~/public_html/Borsari_et_al_transdifferentiation_chromatin/single_figures/fig.8b.pdf", 
    height = 6, width = 5)
ggplot(df.barplot, 
