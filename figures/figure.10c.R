...
function3 <- function(m, degree) {
  ...
  # specified degree of down-regulation
    ...
    for ( G1 in groups1 ) {
      ...
      for ( G2 in groups2 ) {
        ...
        for ( m2 in marks ) {
          ...
          overlap = median(expression.m[rownames(expression.m) %in% m1G1.m2G2, "H168"])
...
# 5. titles for the 4 degrees of downregulation
...
# 7. make plots
# 7.1. 75% downregulation
pdf("~/public_html/Borsari_et_al_transdifferentiation_chromatin/single_figures/fig.10c.75.pdf")
