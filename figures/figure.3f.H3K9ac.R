...
# col_fun = colorRamp2(c(-10, -1, 0.58, 2.32),
#                      c("#c0dbeb", "#d9e9f3", "#f4a582", "#ca0020"))
# 

col_fun = colorRamp2(c(-10, -0.15, 1, 2.32),
                     c("#c0dbeb", "#d9e9f3", "#f4a582", "#ca0020"))
...
heatmap.TSS.pc <- function(mark, class, my.title, group) {
  ...
  # exclude rep 1
  m.H168X1 <- read.table(paste0("H168", mark, "X2.major.", group, ".", class, ".bwtool.matrix.tsv"), h=T, sep="\t", quote=NULL)
...
heatmap.TSS.stable <- function(mark, my.title, group) {
  ...
  # H168X1 - exclude rep. 1
  m.H168X1.upreg <- read.table(paste0("H168", mark, "X2.major.", group, ".upregulation.bwtool.matrix.tsv"), 
                               h=T, sep="\t", quote=NULL)
  m.H168X1.downreg <- read.table(paste0("H168", mark, "X2.major.", group, ".downregulation.bwtool.matrix.tsv"), 
                                 h=T, sep="\t", quote=NULL)
...
p.H3K9ac.upregulation.pc <- heatmap.TSS.pc(mark = "H3K9ac",
                                           class = "upregulation",
                                           group = "positively_correlated",
                                           my.title = "H3K9ac - upreg. - pc")

pdf("~/public_html/Borsari_et_al_transdifferentiation_chromatin/single_figures/fig.3f.H3K9ac.upregulation.positively_correlated.pdf",
    width = 5,
    height = 5)
p.H3K9ac.upregulation.pc
dev.off()


# 2. downreg & positively_correlated
p.H3K9ac.downregulation.pc <- heatmap.TSS.pc(mark = "H3K9ac",
                                             class = "downregulation",
                                             group = "positively_correlated",
                                             my.title = "H3K9ac - downreg. - pc")

pdf("~/public_html/Borsari_et_al_transdifferentiation_chromatin/single_figures/fig.3f.H3K9ac.downregulation.positively_correlated.pdf",
    width = 5,
    height = 5)
p.H3K9ac.downregulation.pc
dev.off()


# 3. upreg + downreg & stable
p.H3K9ac.stable <- heatmap.TSS.stable(mark = "H3K9ac",
                                      group = "stable",
                                      my.title = "H3K9ac - stable")


pdf("~/public_html/Borsari_et_al_transdifferentiation_chromatin/single_figures/fig.3f.H3K9ac.stable.pdf",
    width = 5,
    height = 5)
p.H3K9ac.stable
dev.off()
