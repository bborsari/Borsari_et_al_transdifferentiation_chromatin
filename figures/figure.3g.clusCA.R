...
# out.mca <- clusmca(all.marks.groups, nclus = 3, ndim = 2, method=c("clusCA"),
#         nstart = 100, smartStart = NULL, gamma = TRUE,
#         seed = 1234)


load(".mca.clusCA.RData")
...
colnames(mca_vars_df)[1:2] <- c("X", "Y")
...
main.p <- plot_grid(plotlist = list(p1, p2), nrow=2, ncol=1, rel_heights = c(1, 1))


pdf("~/public_html/Borsari_et_al_transdifferentiation_chromatin/single_figures/fig.3g.pdf", 
    width=8, height=9)
main.p
dev.off()
...
# 8. retrieve clusters
cl1 <- as.data.frame(rownames(mca_genes_df[mca_genes_df$cluster=="cluster1", ]))
cl2 <- as.data.frame(rownames(mca_genes_df[mca_genes_df$cluster=="cluster2", ]))
cl3 <- as.data.frame(rownames(mca_genes_df[mca_genes_df$cluster=="cluster3", ]))

write.table(cl1, "~/public_html/Borsari_et_al_transdifferentiation_chromatin/cluster.1.clusCA.txt", row.names = F, col.names = F, quote=F)
write.table(cl2, "~/public_html/Borsari_et_al_transdifferentiation_chromatin/cluster.2.clusCA.txt", row.names = F, col.names = F, quote=F)
write.table(cl3, "~/public_html/Borsari_et_al_transdifferentiation_chromatin/cluster.3.clusCA.txt", row.names = F, col.names = F, quote=F)
