...
function1 <- function(m) {
  ...
  for ( i in 1:7 ) {
    ...
    for ( k in 1:7 ) {
      ...
      # 3. genes that are down-regulated and positively_correlated for the 
      # two marks
      ...
      # down-regulation of the two marks
  ...
  colnames(df) <- paste(rep(c("anticipated by", "concomitant", "followed by"), 7), 
                        rep(marks, each=3), 
                        sep="_")
  
  # 6. reorder df to have all anticipated groups at the beginning
  df <- df[, c(paste(rep(c("anticipated by", "concomitant", "followed by"), each=7), 
                     rep(marks,3), 
                     sep="_"))]
  rownames(df) <- marks
  ...
  anno_rows <- data.frame(mark2=marks)
...
for ( i in 1:7 ) {
  ...
  # 3.1. read df
  tmp <- read.table(paste0(marks[i], "/QN.merged/ant.del.analysis/decreases/", 
                           marks[i], ".75.50.25.0.tsv"),
                    h=T, sep="\t", stringsAsFactors = F)
  
  # 3.2. retrieve list of down-regulated and positively_correlated genes
  downreg.pc <- read.table(paste0(marks[i], 
                                "/QN.merged/ant.del.analysis/decreases/downregulation.positively_correlated.genes.txt"),
                         h=F, sep="\t", stringsAsFactors = F)
  
  # 3.3. susbet df for the selected genes 
  lom[[i]] <- tmp[rownames(tmp) %in% downreg.pc$V1, ]
...
pdf("~/public_html/Borsari_et_al_transdifferentiation_chromatin/single_figures/fig.9c.75.pdf", width=8)
function1(m="X75_perc")
dev.off()


pdf("~/public_html/Borsari_et_al_transdifferentiation_chromatin/single_figures/fig.9c.50.pdf", width=8)
function1(m="X50_perc")
dev.off()


pdf("~/public_html/Borsari_et_al_transdifferentiation_chromatin/single_figures/fig.9c.25.pdf", width=8)
function1(m="X25_perc")
dev.off()


pdf("~/public_html/Borsari_et_al_transdifferentiation_chromatin/single_figures/fig.9c.0.pdf", width=8)
function1(m="X0_perc")
dev.off()