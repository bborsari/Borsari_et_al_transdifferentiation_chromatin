...
my.function2 <- function(mark) {
  ...
  my.expression <- read.table(paste0(mark, "/QN.merged/expression.matrix.tsv"),
                              h=T, sep="\t")
...
lop[[8]] <- my.legend
lop[[1]] <- lop[[1]] + ylab("expression at 0h - log2 (TPM+1)")

dev.off()

pdf("~/public_html/Borsari_et_al_transdifferentiation_chromatin/single_figures/fig.4t.pdf", width=15, height=3.5)
